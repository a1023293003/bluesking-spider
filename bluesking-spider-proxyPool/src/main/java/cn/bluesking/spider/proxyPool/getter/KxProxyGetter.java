...
public class KxProxyGetter extends BaseProxyGetter {

    /** slf4j日志配置 */
    private static final Logger _LOG = LoggerFactory.getLogger(KxProxyGetter.class);
    
    /** url前缀 */
    private static final String URL_PREFIX = "http://www.kxdaili.com/dailiip/1";
    ...
    List<Proxy> listProxy() {
        try {
            ...
            while (count -- > 0) {
                // 访问主页
                String content = getContent(URL_PREFIX + "/" + pageIndex + ".html#ip", MAX_TRY_PROXY_COUNT);
                ...
                if (CollectionUtil.isEmpty(ips) || CollectionUtil.isEmpty(ports)) {
                    ...
                    _LOG.debug("当前开心代理正在爬取第" + pageIndex + "页代理,爬取到了" + ips.size() + "个ip和" + ports.size() + "个端口");
            ...
            _LOG.error("开心代理获取失败" + e.getMessage());
            return null;
