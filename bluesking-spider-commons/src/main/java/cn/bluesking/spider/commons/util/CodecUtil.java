...
public final class CodecUtil {
     ...
     * 
     * @param x [int]待计算位数整数
     * @return [int]该方法返回计算位数的长度,当传入整数是负数的时候,该方法返回1
     */
     ...
     * </pre>
     * 
     * @param source [String]待编码数据源
     * @return [String]该方法执行成功后返回特殊字符已经被编码的数据源,当输出源中包含中文字符(ASCII码为负数的字符)的时候抛出参数不合法异常
     */
    public static final String encodeHTML(String source) {
        return encodeHTML(source, false);
    }
     ...
     * 
     * @param source [String]待编码数据源
     * @param allKill [boolean]是否全部字符都进行转码,该参数值为true时表示对所有字符进行编码,该参数为false时表示仅对特殊字符进行编码
     * @return [String]该方法执行成功后返回特殊字符已经被编码的数据源,当输出源中包含中文字符(ASCII码为负数的字符)的时候抛出参数不合法异常
     */
    public static final String encodeHTML(String source, boolean allKill) {
     ...
     * 解码html中的&#xxx;格式的字符
     * 
     * @param source  [String]待解码数据源
     * @return [String]该方法执行成功后返回特殊编码已经被解码的数据源
     */
            ...
            if (aChar == '&') {
                ...
                if (off < end) {
                    ...
                    if (StringUtil.isNumber(aChar)) {
                        isChar = false;
                    } else if (StringUtil.isLetter(aChar)) {
                        isChar = true;
                    } else {
                        isChar = null;
                    }
     ...
     * 
     * @param source [String]待解码数据源
     * @return [String]该方法执行成功后会把数据源中\\uXXXX格式的Unicode编码解码成对应的字符,并返回解码后的数据源
     */
     ...
     * 将URL编码
     * 
     * @param source [String]待编码的数据源
     * @return [String]该方法执行成功后会返回进行base64加密后的数据源,否则抛出运行异常
     */
     ...
     * 
     * @param source [String]待解码数据源
     * @return [String]该方法执行成功后会返回进行base64解码后的数据源,否则抛出运行异常
     */
     ...
     * MD5加密
     * 
     * @param source [String]待加密的数据源
     * @return [String]该方法将返回进行MD5加密后的数据源
     */
